...
public class OptiLockThread extends Thread{
	//从连接池中得到jedis
	private redis.clients.jedis.Jedis Jedis= MyJedisPool.getJedis();
		...
		if (num>0&&num<=200) {
			...
			if (list!=null&&list.size()>0) {
				...
				for (Object object : list) {
					System.out.println(customerName+"秒杀成功商品:"+(200-Integer.valueOf(object.toString()))+"  当前商品剩余数:"+object.toString());
