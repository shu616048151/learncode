...
public class LevelDBDemoTest {
    private static final String PATH = "D:/data/leveldb";
    ...
    public void batchInsert(){
        ...
        for (int n=1;n<=5;n++){
            ...
            try {
                ...
                for (int i = 0; i < num; i++) {
                    byte[] key = RandomUtil.randomStr(16).getBytes(CHARSET);
                    byte[] value = RandomUtil.randomStr(100).getBytes(CHARSET);
                    long startTime = System.currentTimeMillis();
                    db.put(key, value);
                    long entTime = System.currentTimeMillis();
                    consume += entTime - startTime;
                }
                long end = System.currentTimeMillis();
                System.out.println("--------------------------第"+n+"次------------------------");
                System.out.println("插入数据条数:" + num + " 消耗时间(不包含产生随机key和value) :" + consume);
                System.out.println("插入数据条数:" + num + " 消耗时间(包含产生随机key和value) :" + (end - start));
            } catch (IOException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            } finally {
...
//            }
        ...
        System.out.println("平均消耗时间:"+(avgConsume/5));
    }
    ...
    @Test
    public void randomReadFromSnapshotTest() {
        DBFactory factory = new Iq80DBFactory();
        File file = new File(PATH);
        Options options = new Options();
        DB db = null;
